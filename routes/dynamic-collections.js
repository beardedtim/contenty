...
Dynamic.get('/:slug', middleware.route(async (req, res) => {
  const query = {
  ...
  res.json({ data })
})).post('/:slug', middleware.route(async (req, res) => {
  const { body } = req

  const data = await UseCases.Dynamic.create(req.params.slug, body, db)

  res.status(201).json({ data })
})).get('/:slug/:id', middleware.route(async (req, res) => {
  const data = await UseCases.Dynamic.getById(req.params.id, db)

  if (!data) {
    throw new HTTPErrors.ResourceNotFound(`${req.params.slug}::ID::${req.params.id}`)
  }

  res.json({ data })
})).patch('/:slug/:id', middleware.route(async (req, res) => {
  const data = await UseCases.Dynamic.updateById(req.params.id, req.body, db)

  res.json({ data })
})).delete('/:slug/:id', middleware.route(async (req, res) => {
