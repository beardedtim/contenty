...
const getCollection = async (slug, db) => {
  const collection = await db.from('collections').where({ slug }).first().select('*')
...
module.exports.create = async (collectionSlug, item, db) => {
  ...
  const [created] = await db.into('items').insert({
...
const getById = (id, db) => db.from('items').where({ id }).select(['id', 'data']).first()
module.exports.getById = getById

module.exports.updateById = async (id, update, db) => {
  const item = await getById(id, db)

  const [updated] = await db.from('items').where({ id }).update({
    data: {
      ...item.data,
      ...update
    }
...
module.exports.deleteById = async (id, db) => {
  const [removed] = await db.from('items').where({ id }).del().returning(['id', 'data'])
